                        ...
                        for sdir, dirs, files in os.walk(dfile):
                                        ...
                                        while self.reqNewNm == "":
                                            ...
                                            # 3 new name
                                                ...
                                                try:
                                                    ...
                                                    # funziona non ho uso di CPU
                                                    while self.reqNewNm == "":
                                                        time.sleep(1)
                                                    else:
                                                        # se -1 vuol dire che ho premuto cancel nel dialogo di rinomina
                                                        if self.reqNewNm == -1:
                                                            items_skipped += "{}:\n{}\n------------\n".format(origdir, "Skipped.")
                                                        #
                                                        #self.sig.emit([sdir, incr_size/total_size, total_size])
                            ...
                            for item in files:
                                ...
                                if os.path.exists(destfpath):
                                    ...
                                    while self.reqNewNm == "":
                                        ...
                                        # 2 overwrite
                                            ...
                                            try:
                                                ...
                                                # se destinazione link (no broken link)
                                    ...
                                    dcode = 0
                                #
                                # the item doesn't exist or it is a broken link
                                else:
                                    # if broken link
                                    if os.path.islink(destfpath):
                                        try:
...
class IconProvider(QFileIconProvider):
    ...
    def evaluate_pixbuf(self, ifull_path, imime):
        ...
        if hmd5 != "Null":
            if LARGETHUMS:
                file_icon = QIcon(QPixmap(XDG_CACHE_LARGE+"/"+str(hmd5)+".png").scaled(THUMB_SIZE, THUMB_SIZE, Qt.KeepAspectRatio, Qt.FastTransformation))
            else:
