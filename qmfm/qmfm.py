...
class copyThread2(QThread):
    ...
    # for calculating the folder size 
    def folder_size(self, path):
        total_size = 0
        for dirpath, dirnames, filenames in os.walk(path):
            for fl in filenames:
            ...
            # dir
                ...
                if dfile != tdest:
                    ...
                    if not os.path.exists(tdest):
                        ...
                        try:
                            self.sig.emit(["mSending", os.path.basename(dfile)])
                            # alternative one: rename the broken symlink at destination
                            if os.path.islink(tdest):
                                ret = self.faddSuffix(tdest)
                                ...
                                if action == 1:
                                    ...
                                    shutil.move(dfile, tdest)
                            ## alternative two: dialog
                            ## if there is a broken link at destination
                            #if os.path.islink(tdest):
                                ## overwrite
                                #if self.atype == 2:
                                    #os.unlink(tdest)
                                ## new name
                                    ...
                                    #while self.reqNewNm == "":
                                        ...
                                        ##
                                        #shutil.move(tdest, os.path.join(self.pathdest,self.reqNewNm))
                                    #self.reqNewNm = ""
                                ## automatic suffix
                                #elif self.atype == 4:
                                    #ret = self.faddSuffix(tdest)
                    ...
                    #
                        ...
                        lItems = os.listdir(dfile)
                        for iitem in lItems:
                            try:
                                if not os.path.isdir(iitem):
                                    if newDestDir != "":
                                        ttd = os.path.join(os.path.dirname(tdest),newDestDir)
                                    else:
                                        ttd = tdest
                                    if action == 1:
                                        shutil.copy2(os.path.join(dfile,iitem), ttd, follow_symlinks=False)
                                    elif action == 2:
                                        shutil.move(os.path.join(dfile,iitem), ttd)
                            except Exception as E:
                                items_skipped += "{}:\n{}\n------------\n".format(os.path.join(dfile,iitem), str(E))
                        #
                        for sdir,ddir,ffile in os.walk(dfile):
                            for dr in ddir:
                                origDir = os.path.join(sdir, dr)
                                if newDestDir != "":
                                    destdir = newDestDir
                                else:
                                    destdir = sdir
                                todest = os.path.join(destdir,os.path.join(sdir,dr)[len(os.path.join(self.pathdest, os.path.basename(dfile)))+1:])
                                os.makedirs(todest)
                                # the atype code
                                dcode = 0
                                for sitem in ffile:
                                    # full path at destination
                                    itemAtDest = os.path.join(todest,sitem)
                                    fpsitem = os.path.join(origDir, sitem)

                                    if os.path.exists(itemAtDest) or os.path.islink(itemAtDest):
                                        # link at destination
                                        if os.path.islink(itemAtDest):
                                            os.unlink(itemAtDest)
                                            items_skipped += "{}:\n{}\n------------\n".format(itemAtDest, "Broken link. Deleted")
                                        if os.path.exists(itemAtDest):
                                            #
                                            ...
                                            while self.reqNewNm == "":
                                                ...
                                                # 2 overwrite
                                                    ...
                                                    try:
                                                        if os.path.islink(itemAtDest):
                                                            os.unlink(itemAtDest)
                                                        elif os.path.isdir(itemAtDest):
                                                            shutil.rmtree(itemAtDest)
                                                        if action == 1:
                                                            shutil.copy2(fpsitem, itemAtDest, follow_symlinks=False)
                                                        elif action == 2:
                                                            shutil.move(fpsitem, itemAtDest)
                                                        #
                                                    except Exception as E:
                                                ...
                                                # 3 new name - 4 automatic
                                                    ...
                                                    try:
                                                        ret = self.faddSuffix(itemAtDest)
                                                        iNewName = os.path.join(os.path.dirname(itemAtDest),ret)
                                                        if action == 1:
                                                            shutil.copy2(fpsitem, iNewName, follow_symlinks=False)
                                                        elif action == 2:
                                                            shutil.move(fpsitem, iNewName)
                                                        items_skipped += "{}:\nRenamed:\n{}\n------------\n".format(itemAtDest, ret)
                                                        #
                                                    except Exception as E:
                                                        items_skipped += "{}:\n{}\n------------\n".format(fpsitem, str(E))
                                    #
                                        ...
                                        try:
                                            if action == 1:
                                                shutil.copy2(fpsitem, todest, follow_symlinks=False)
                                            elif action == 2:
                                                shutil.move(fpsitem, todest)
                                        except Exception as E:
                                            items_skipped += "{}:\n{}\n------------\n".format(fpsitem, str(E))
                    #
                    # action 2
...
class copyItems2():
    ...
    def threadslot(self, aa):
        ...
        # the copying process ends
            ...
            self.button1.setEnabled(True)
            self.button2.setEnabled(False)
            # something appens with some items
            if len(aa) == 4 and aa[3] != "":
                MyMessageBox("Info", "Some errors with some items", "", aa[3])
    ...
    def doubleClick(self, index):
        ...
        if os.path.isdir(path):
            ...
            if defApp != "None":
                ...
                MyDialog("Error", "No program found.")
