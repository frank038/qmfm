                ...
                if dfile != tdest:
                    ...
                    # exists at destination
                        ...
                        if newDestDir != "":
                            ...
                            ## self.pathdest è la directory a destinazione in cui copiare la cartella base 
                            destdir = tdest
                        #
...
class MainWin(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        if os.path.dirname(parg) == "":
            ...
            if os.path.exists(parg):
                self.openDir(parg, 1)
            else:
                # cycle throw the path backward to find an existent directory
...
class LView(QBoxLayout):
    ...
    def __init__(self, TLVDIR, window, flag, parent=None):
        ...
        # reload the view by pressing the R key
        self.clickable(self.listview).connect(self.freloadView)
    ...
    def fOpenFolder(self):
        if self.selection:
            itemSelectedPath = self.fileModel.fileInfo(self.selection[-1]).absoluteFilePath()
        else:
            return
    ...
    # reload the view - just reopen the same folder in the view - function
    def freloadView(self):
        try:
            self.listview.clearSelection()
            MMTAB.setTabText(MMTAB.currentIndex(), os.path.basename(self.lvDir))
            MMTAB.setTabToolTip(MMTAB.currentIndex(), self.lvDir)
    ...
    # reload the view - just reopen the same folder in the view
    def clickable(self, widget):
        class Filter(QObject):
            clicked = pyqtSignal()
            def eventFilter(self, obj, event):
    ...
    def ftrashAction(self):
        if self.selection:
            ...
            if len(self.selection) < ITEMSDELETED:
                ...
                ret = retDialogBox("Info", "Do you really want to move this item(s) to the trashcan?", "", dialogList)
            else:
                ret = retDialogBox("Info", "Do you really want to move this item(s) to the trashcan?", "", "Too many items.\n")
            #
            if ret.getValue():
