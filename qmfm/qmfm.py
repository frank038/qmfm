...
class LView(QBoxLayout):
    ...
    def appByMime(self, path):
        ...
        for idesktop in lAdded:
            ...
            for ddir in xdgDataDirs:
                ...
                # or find it in the HOME
                elif idesktop in os.listdir(os.path.join(os.path.expanduser("~"),".local/share/applications")):
                    desktopPath = os.path.join(os.path.expanduser("~"),".local/share/applications", idesktop)
            #
            mimeProg2 = DesktopEntry(desktopPath).getExec()
            if mimeProg2:
                mimeProg = mimeProg2.split()[0]
    ...
    # function that returns mimetypes added, removed in the mimeappss.list
    def addMime(self, mimetype):
        
        # mimeapps.list can have three not mandatory sectors
    ...
    def onRightClick(self, position):
        ...
        if vr:
            ...
            # delete function - non mi ricordo i valori si self.flag
            if USE_DELETE:
                if self.flag != 2:
...
############# Paste and Merge section
    ...
    def fnewList2(self, filePaths, dest_path):
        ...
        items_skipped = ""
        for ditem in filePaths:
            # link e broken link (se non broken link os.path.isfile basta)
