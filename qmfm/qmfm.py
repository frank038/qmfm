...
class MainWin(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # history button
        if SHOW_HISTORY:
            # scegliere una icona dal tema
            self.hbutton = QPushButton(QIcon.fromTheme("window-closea"), "")
            self.hbutton.setToolTip("History")
            self.obox1.addWidget(self.hbutton, 0, Qt.AlignRight)
            self.hbmenu = QMenu()
            self.hbutton.setMenu(self.hbmenu)
            self.hbmenu.triggered.connect(self.fhbmenuction)
    ...
    def fhbmenuction(self, action):
        folder_in_history_clicked = action.text()
        # open the folder in a new tab
        if os.path.exists(folder_in_history_clicked):
            if os.access(folder_in_history_clicked, os.R_OK):
                self.openDir(folder_in_history_clicked, 0)
                return
        #
...
class LView(QBoxLayout):
    ...
    def __init__(self, TLVDIR, window, flag, parent=None):
        ...
        if SHOW_HISTORY:
            self.window.hbmenu.addAction(self.lvDir)
