    ...
    def item_op(self):
        ...
        for dfile in newList:
            ...
            if os.path.isdir(dfile):
                ...
                #
                # 1 skip - 2 replace - 3 new name - 4 automatic - 5 backup
                dcode = 0
            ...
            # file or link/broken link or else
                ...
                if dfile != tdest:
                    if os.path.exists(tdest):
                        if self.atype == -4:
                            self.atype = pasteNmergeDialog(tdest, "The file {} exists.\nWhat do you want to do?".format(os.path.basename(tdest))).getValue()
                        # 1 skip
                        if self.atype == 1:
                            ...
                            try:
                                ...
                                # if os.path.islink(dfile):
                                    # if not os.path.exists(os.readlink(dfile)):
                                        ...
                                        # continue
                                # # remove the link to be overwritten
                                # if os.path.islink(tdest):
                                    # os.unlink(tdest)
                                # 
                                if action == 1:
    ...
    def onRightClick(self, position):
        ...
        ## background
            ...
            if USE_PM != 2:
                if self.flag == 1 or self.flag == 3:
                    menu.addSeparator()
                    pastAction = QAction("Paste only", self)
