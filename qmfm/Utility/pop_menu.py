    ...
    def fpop_lists(self):
        ...
        for item in  self.info_desktop:
            ...
            if ret:
                ...
                list_one.append([item.name, item.path, idx, pexec, icon])
            else:
                ...
                if ret:
                    ...
                    if pexec[0:5] == "$HOME":
                        pexec = "~"+pexec[5:]
                    icon = DesktopEntry.DesktopEntry(item.path).getIcon()
                    list_two.append([item.name, item.path, idx, pexec, icon])
            
        ## sort the lists
        # sorted by name
        list_one2 = sorted(list_one, key=lambda list_one: list_one[0].lower(), reverse=False)
        # sorted by category
