            ...
            with open(f,"r") as ff:
                ...
                while fline:
                    if fline.lstrip(" ")[0] != "#":
                        tmp = fline.rstrip("\n").split(":")
                        # with weight
                        #LIST_FILES.append([tmp[1],tmp[2].replace("*.",""),tmp[0]])
                        # without weight
...
class MainWin(QWidget):
    ...
    def findItemInListaAndUpdate(self, item):
        ...
        # remove the markers
        if itemText2[-5:] == " (+*)":
            itemText = itemText2[:-5]
        elif itemText2[-4:] == " (+)":
            itemText = itemText2[:-4]
        elif itemText2[-4:] == " (-)":
            itemText = itemText2[:-4]
        #
        if itemText == "":
        ...
        if in_lDA == 0:
            ...
            for el in self.lAA:
                if self.imime+"=" in el:
                    if itemDname in el:
                        newEl = self.fremoveText(el,itemDname)
                        # 721 phototonic.desktop; senza newline
                        #
                        if newEl:
                            newEel = self.imime+"="+newEl
                            # remove the old entry from the list
